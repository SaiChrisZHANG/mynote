...
\begin{itemize}
    ...
    \item A variant of SBM is degree corrected SBM which incorporates the degree heterogeneity. Each node is assigned a parameter $\theta_i >0$ such that $\Pr \left(\mathbf{X}_{ij}=1\right) = \theta_i\theta_j \mathbf{P}_{C(i),C(j)}$ and $\mathbf{H} = \boldsymbol{\Theta\Pi}\mathbf{P}\boldsymbol{\Pi'\Theta}$, where $\boldsymbol{\Theta} = \diag \left(\theta_1,\cdots,\theta_n\right)$
\end{itemize}
