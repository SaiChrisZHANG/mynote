...
\begin{example}{Dimension Reduction and Variable Selection via Sparse SVD}{sparse_svd}
    ...
    $$
    where $\mathbf{u}_1,\mathbf{u}_2,\mathbf{u}_3$ are all $p\times 1$, while $\mathbf{v}_1,\mathbf{v}_2,\mathbf{v}_3$ are all $q\times 1$, hence, there are only $3\times (1000+100+1) = 3303$ paramaters to estimate. If futher assume sparcity, the dimension would be even lower.
\end{example}
