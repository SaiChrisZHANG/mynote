...
where 
\begin{itemize}
    \item $\rho_a(\cdot),\rho_b(\cdot)$ are penalty functions with regularization parameters $\lambda_d,\lambda_a,\lambda_b\geq 0$
    \item $\lVert \cdot \rVert _F$ is the nuclear norm, defined as the \textbf{sum} of its singular values $\lVert \mathbf{A}\rVert _F =\sum_i\sigma_i(\mathbf{A})$
\end{itemize}
