...
\begin{itemize}
    ...
    \item \myhl[myblue]{\textbf{Sufficient impurity decrease}}: $\exists \alpha_1\geq 1$ s.t. $\forall \mathbf{t} = t_1\times \cdots \times t_p$, $$ \mathrm{Var}\left[m(\mathbf{X})\mid \mathbf{X}in\mathbf{t}\right] \leq \alpha_1 \sup_{j\in \left\{1,\cdots, p \right\}, c\in t_j} \left(\mathbb{II}\right)_{\mathbf{t,t}(j,c)}$$
\end{itemize}
