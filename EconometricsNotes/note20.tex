...
$$
the size of $\mathrm{SID}(\alpha)$ is \textbf{non-decreasing} in $\alpha \geq 1$: if $m(\mathbf{X})\in \mathrm{SID}(\alpha -c)$ for some $\alpha-c\geq 1$ and $c>0$, then $m(\mathbf{X})\in \mathrm{SID}(\alpha)$
