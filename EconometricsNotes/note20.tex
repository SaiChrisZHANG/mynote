...
\begin{theorem}{Consistency}{main_consistency}
    Let $0<b\leq 1,0<\gamma_0\leq 1,\alpha_2 >1,0<\eta<1/8,0<c<1/4$ and $\delta>0$, $2\eta < \delta <\frac{1}{4}$. Let $A=\left\{ a_1,\cdots,a_B \right\}$ with $\# a_i = \lceil bn \rceil $ for $i=1,\cdots,B$ and $a\in A$. Then $\exists C>0$   
\end{theorem}
