...
\begin{itemize}
    \item[1] start with the null model $\hat{\boldsymbol{\beta}}=\mathbf{0}$: $\hat{\mu} = \mathbf{X}'\mathbf{0}=0$ 
    \item[2] calculate residual vector $\mathbf{r} = \mathbf{y}-\hat{\mu}$
    \item[3] determine the correlation vector between $\mathbf{r}$ and each parameter $\mathbf{x}_j,\forall j=1,\cdots,p$
\end{itemize}
