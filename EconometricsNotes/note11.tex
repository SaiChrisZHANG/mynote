...
Mathematically, Lasso is quite intuitive, but computionally, it can be quite consuming. \citet{efron2004least} propose an alogrithm that takes steps from a all-0 model to the biggest model (OLS), that is, \textbf{Least Angle Regression (LARS)}. 

\subsubsection*{Intuition}
The basic intuition of LARS is quite straight-forward: covariates are considered from the \myhl[myred]{\textbf{\textit{highest}}} corerlation with $\mathbf{y}$ (\textit{smallest} angle from $\mathbf{y}$) to the \myhl[myblue]{\textbf{\textit{least}}} correlated one (\textit{largest} angle from $\mathbf{y}$).
...
\begin{figure}[ht]
    ...
    \centering
      \begin{tikzpicture}[scale=1.4]
          ...
          \draw [color=myred, ultra thick] (0,0) -- (2.5,0) node[below] {Step 1};
          \draw [color=myblue, ultra thick] (2.5,0) -- (3.5,1.5) node[below,xshift=15pt] {Step 2};
      \end{tikzpicture}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        ...
        \begin{tikzpicture}
            ...
            % draw LARS steps
            \draw[LARS, myred] (O) -- (0.45,0,0) node[left,xshift=-10pt] {Step 1};
            \draw[LARS, myblue] (0.45,0,0) -- (0.6,0.45,0) node[below] {Step 2};
            \draw[LARS, teal] (0.6,0.45,0) -- (P) node[above] {Step 3};
        \end{tikzpicture}
    \end{minipage}
\end{figure}
