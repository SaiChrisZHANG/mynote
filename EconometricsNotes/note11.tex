...
\end{align*}
then we have 
\begin{lemma}{Naive elastic net solution}{elastic_net}
    ...
    $$
\end{lemma}
Elastic Net enjoys 2 advantages:
\begin{itemize}
...
\begin{align*}
    \hat{{\beta}}_j\left(\text{naive elastic net}\right) &= \frac{\left( \left\vert \hat{\beta}_i^{OLS}\right\vert -\frac{\lambda_1}{2} \right)_+}{1+\lambda_2} \cdot \mathrm{sign}\left[ \hat{\beta}_j^{OLS}\right] \\
    \hat{{\beta}}_j\left(\text{ridge}\right) &= \frac{\hat{\beta}_j^{OLS}}{1+\lambda_2}\\
    \hat{\beta}_j \left(\text{lasso}\right) &= \left( \left\vert \hat{\beta}_i^{OLS}\right\vert -\frac{\lambda_1}{2} \right)_+\cdot \mathrm{sign}\left[ \hat{\beta}_j^{OLS}\right]
\end{align*}
...
\begin{figure}[ht]
    ...
    \begin{minipage}[b]{0.45\textwidth}
        ...
        \begin{tikzpicture}[scale=1.6]
            ...
            \draw [color=myred, very thick] (0.5,1.5) -- (1.5,2) -- (2.5,2) -- (3.5,2.5) node[right]{elastic net};
        \end{tikzpicture}
    \end{minipage}
\end{figure}
