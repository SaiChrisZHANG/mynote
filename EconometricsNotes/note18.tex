...
\begin{theorem}{Asymptotic Distribution of Spiked Eigenvalues and Eigenvectors}{spike_eigen_asymptotics}
    ...
    \begin{itemize}
        ...
        \item \myhl[myblue]{\textbf{Eigenvectors}}: if the unit vector $\mathbf{u}$ satisfies that $\left\vert \mathbf{u}'\mathbf{v}_k \right\vert \in \left[0,1-\epsilon\right]$ for some $\epsilon>0$, and $\sigma_{\min}^{-1}\left\Vert \mathbf{v}_k\left(\mathbf{b}_{\mathbf{u,k,t}_k}'-\mathbf{u}'\mathbf{v}_k\mathbf{v}_k'\right) \right\Vert_{\infty} \rightarrow 0$, then 
    \end{itemize}
\end{theorem}
