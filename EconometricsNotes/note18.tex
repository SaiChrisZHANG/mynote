...
\begin{document}
\lecture{18}{Eigenvalue and Spike Models}{}{Sai Zhang}{A spike model can be used to determine the latent data structure.}{The note is built on Prof. \hyperlink{http://faculty.marshall.usc.edu/jinchi-lv/}{Jinchi Lv}'s lectures of the course at USC, DSO 607, High-Dimensional Statistics and Big Data Problems.}
...
\begin{itemize}
    ...
    \item \myhl[myblue]{\textbf{Multiple spikes}}: the asymptotic analysis can be extended to a \textbf{fixed} number of sub-critical spikes. In the case of slowly increasing number of spikes, relatively little is known about the phase transition phenomenon. It is not clear whether the experiments of observing sample covariance eigenvalues corresponding to the null case and an alternative with a growing number of sub-critical eigenvalues remain mutually contiguous. 
    ...
    \item \myhl[myblue]{\textbf{Relaxing} the assumption $p\leq n_1$}: 
    \begin{itemize}
...
The goal is to, with
\begin{itemize}
    \item the \myhl[myblue]{\textbf{latent eigenstructure}} $\left(\mathbf{D},\mathbf{V}\right)$
    \item the \myhl[myblue]{\textbf{data matrix}} $\mathbf{X}$
\end{itemize}
...
\begin{theorem}{Asymptotic Distribution of Spiked Eigenvalues and Eigenvectors}{spike_eigen_asymptotics}
    ...
    \begin{itemize}
        \item \myhl[myblue]{\textbf{Eigenvalues}} $$ \frac{\lambda_k - t_k - \mathbb{E}\left(\mathbf{v}_k'\mathbf{W}\mathbf{v}_k\right)}{\left[\mathrm{var}\left(\mathbf{v}_k'\mathbf{W}\mathbf{v}_k\right)\right]^{1/2}} \xrightarrow{d}\mathcal{N}(0,1) $$
        \item \myhl[myblue]{\textbf{Eigenvectors}}: if the unit vector $\mathbf{u}$ satisfies that $\left\vert \mathbf{u}'\mathbf{v}_k \right\vert \in \left[0,1-\epsilon\right]$ for some $\epsilon>0$, and $\sigma_{\min}^{-1}\left\Vert \mathbf{v}_k\left(\mathbf{b}_{\mathbf{u,k,t}_k}'-\mathbf{u}'\mathbf{v}_k\mathbf{v}_k'\right) \right\Vert _{\infty} \rightarrow 0$, then 
        \begin{align*}
            \frac{t_k\left( \mathbf{u}'\hat{\mathbf{v}}_k + A_{\mathbf{u},k,t_k}\tilde{\mathcal{P}}^{1/2}_{k,t_k} \right) - \mathbb{E}\left[ \left( \mathbf{b}_{\mathbf{u},k,t_k}' - \mathbf{u}' \mathbf{v}_k\mathbf{v}_k' \right) \mathbf{Wv}_k \right] }{ \left\{ \mathrm{var} \left[ \left( \mathbf{b}_{\mathbf{u},k,t_k}' - \mathbf{u}' \mathbf{v}_k\mathbf{v}_k' \right) \mathbf{Wv}_k \right] \right\}^{1/2}} &\xrightarrow{d}\mathcal{N}(0,1)\\
            \frac{2t_k^2 \left( \mathbf{v}_k'\hat{\mathbf{v}}_k + A_{\mathbf{u},k,t_k} \tilde{\mathcal{P}}^{1/2}_{k,t_k} \right)}{ \left[ \mathrm{var}\left( \mathbf{v}_k' \mathbf{W}^2 \mathbf{v}_k \right) \right]^{1/2} } &\xrightarrow{d} \mathcal{N}(0,1)
        \end{align*}
...
\end{align*}
where $\mathbf{l}$ is a vector with the first $n_1$ entries being 1 and rest 0, $q\in (p,1]$, and 
\begin{itemize}
    \item under the null, the top eigenvector is $\mathbf{v}_1 = \mathbf{1}/\sqrt{n}$ and thus 
    \begin{align*}
        \mathbb{E}\left( \mathbf{v}_1'\mathbf{W}^2\mathbf{v}_1 \right) & =np(1-p)\\
        \mathrm{var} \left( \mathbf{v}_1'\mathbf{W}^2\mathbf{v}_1 \right)&= p(1-p)\left[2(n-1)+p^3+(1-p)^3\right]
    \end{align*}
    \item the natural test statistic based on Thm.\ref{thm:spike_eigen_asymptotics} is given by 
    \begin{equation*}
        T_n \coloneq \frac{2\lambda_1^2 \left(\mathbf{1'\hat{v}}_1/\sqrt{n} -1\right) + n\hat{p}(1-\hat{p}) }{ \sqrt{\hat{p}(1-\hat{p})\left[2(n-1)+\hat{p}^3+(1-\hat{p})^3\right]} } \xrightarrow{d}\mathcal{N}(0,1)
    \end{equation*}
    where $\hat{p} = \frac{1}{n(n-1)}\sum_{1\leq i\leq j \leq n} x_{ij}$
\end{itemize}

\paragraph*{Rank inference}
Consider the hypothesis
\begin{align*}
    H_0 &: K=K_0 & H_1 &: K>K_0
\end{align*}
where $K$ is the true \textbf{rank} of the mean matrix $\mathbf{H}$ and $K_0$ is some pre-specified positive integer satisfying $K_0 \leq K $, define
$$
\hat{w}_{ij} = x_{ij} - \sum^{K_0}_{i=1}\lambda_k \mathbf{e}'i \hat{\mathbf{v}}_k \hat{\mathbf{v}}_k' \mathbf{e}_j
$$
under the null and some additional regularity conditions, $\hat{w}_{ij}$ is expected to be close to $w_{ij}$, hence the test statistic can be given by
$$
T_n \coloneq \frac{\sum^n_{i=1}\hat{w}_{ii}}{\sqrt{\sum^n_{i=1}\hat{w}_{ii}^2}} \xrightarrow{d} \mathcal{N}(0,1)
$$
