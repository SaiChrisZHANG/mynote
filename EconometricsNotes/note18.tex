...
\end{align}
which can be denoted as $\mathbf{n},p\rightarrow_{\boldsymbol{\gamma}}\infty$ where $\mathbf{n}=\left\{ n_1,n_2 \right\}$ and $\boldsymbol{\gamma}=\left\{\gamma_1,\gamma_2 \right\}$. Under the null (the true value of the spike $\theta_0=0$), $\lambda_1,\cdots,\lambda_p$ are the eigenvalues of
\begin{itemize}
...
A summary of the 3 classes of distributions is shown in Table 
...
\begin{table}[ht]
    ...
    \begin{center}
      \begin{tabular}{lccccc}
        Case & $F^{\lim}_{\boldsymbol{\gamma}}$ & Density, $\lambda\in[\beta_{-},\beta_+]$ & $\beta_{\pm}$ & Threshold $\hat{\theta}$ \\
        \hline
        SMD & SC & $\frac{R(\lambda)}{2\pi}$ & $\pm 2$ & $1$\\
        PCA & MP & $\frac{R(\lambda)}{2\pi\gamma_1 \lambda}$ & $(1\pm \sqrt{\gamma_1})^2$ & $\sqrt{\gamma_1}$\\
        SigD & $_{\mathrm{1}}\mathcal{F}_{\mathrm{0}}$ & $(1+\theta)^{-n_1/2}$ & - & - & $\theta n_1/(n_2(1+\theta))$ \\
        REG$_0$ & $_{\mathrm{0}}\mathcal{F}_{\mathrm{1}}$ & $\exp(-n_1\theta/2)$ & - & $n_1/2$ & $\theta n_1^2/4$ \\
        REG & $_{\mathrm{1}}\mathcal{F}_{\mathrm{1}}$ & $\exp(-n_1\theta/2)$ & $n/2$ & $n_1/2$ & $\theta n_1^2/(2n_2)$ \\
        CCA & $_{\mathrm{2}}\mathcal{F}_{\mathrm{1}}$ & $(1+n_1\theta/n)^{-n/2} $ & $(n/2,n/2)$ & $n_1/2$ & $\theta n_1^2/(n_2^2 + n_2n_1(1+\theta))$
