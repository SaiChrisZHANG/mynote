...
\begin{itemize}
    ...
    \item The maximum likelihood estimator for the spike $\hat{\theta}_{ML}$. The limiting process $X(\theta)$ achieves maximum at a unique point with probability one. Therefore by the argmax continuous mapping theorem, $\hat{\theta}_{ML}$ converges in distribution to the argmax of $X(\theta)$. The distribution of such an argmax can also be approximated via simulations.
\end{itemize}
