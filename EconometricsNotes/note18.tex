...
\begin{lemma}{Contour Integral Representations for Likelihood Ratios}{contour_integral_represent}
    ...
    \end{equation}
    where $s=p/2-1$, the values of $\alpha(\theta),\boldsymbol{\Phi}_{11},a,b,\mathrm{p},\mathrm{q}$ for difference cases are given as 

    and $a-s$, $b-s$ are vectors with elements $a_j-s$, $b_j-s$ respectively, $$ q_s = \prod^{\mathrm{p}}_{j=1}\frac{\Gamma (a_j-s)}{\Gamma (a_j)} \prod^{\mathrm{q}}_{i=1}\frac{\Gamma (b_i)}{\Gamma (b_i-s)} $$
\end{lemma}
