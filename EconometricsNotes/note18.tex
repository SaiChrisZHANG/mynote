...
\begin{theorem}{Asymptotics of LR}{asymptotic_lr}
    Suppose that the null hypothesis holds, $\theta_0= 0$. Let $\bar{\theta}$ be the threshold corresponding to $F_{\boldsymbol{\gamma}}$ as in Tab.\ref{tab:3classes_distributions}, let $\epsilon$ be an arbitrarily small fixed positive number, then $\forall \theta\in (0,\bar{\theta}-\epsilon]$, as $\mathbf{n},p\xrightarrow{\boldsymbol{\gamma}} \infty$
\end{theorem}
