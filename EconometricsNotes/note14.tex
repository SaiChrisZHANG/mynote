...
\begin{itemize}
    \item \textcolor{myblue}{\textbf{Error tail distribution}}: $\Pr (\mathcal{E}^c) = O(p^{-c_1})$ and $\Pr (\mathcal{E}^c_0) = O(n^{-c_1})$ for some positive constant $c_1$ that can be sufficiently large for large enough $c_0$
    \item \textcolor{myblue}{\textbf{Bounded variance}}: $b(\theta)$ satisfies that $c_2\leq b''(\theta)\leq c_2^{-1}$ in its domain, where $c_2$ is some positive constant
\end{itemize}
