...
\begin{itemize}
    ...
    \item \myhl[myblue]{\textbf{Separately exchangeable arrays}} For the case of separately exchangeable arrays (the $n$ units stem from $k$ \textbf{different} populations, or multiway clustering as in \citet{menzel2021bootstrap}), then we must assume a stronger version of Assumption \ref{assump:jexch_diss_array}
    ...
    \begin{theorem}{Bootstrap Convergence: Separately Exchangeable Arrays}{bootstrap_conv_sepexcharray}
        ...
        \begin{itemize}
            ...
            \item[2] If (B-i) of Assumption \ref{assump:assumptions_on_f} holds, the process $\mathbb{G}_n$ coverges weakly in $l^{\infty}(\mathcal{F})$ to a centered Gaussian process $\mathbb{G}_{\lambda}$ on $\mathcal{F}$ as $n$ tends to infinity
        \end{itemize}
    \end{theorem}
\end{itemize}
