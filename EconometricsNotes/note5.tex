...
\begin{assumption}{Assumptions on the Distribution of $\left\{Z_{gi}\right\}_{g,i}$}{sasakiwang2022assump1}
    \begin{itemize}
        \item $\left\{Z_{gi}\right\}_{g,i}$ is identically distributed with CDF $F$, which is \textbf{RV} at infinity with $\alpha >1$\footnote{$Z_{gi}$ should have a finite mean, and a regularly varying tail.}
        \item The copula density $c(u_1,\cdots,u_n) = \partial^n C(u_1,\cdots, u_n)/\partial u_1\cdots \partial u_n$ exists and is \textbf{uniformly bounded}
        \item $N_g$ is independent of $\left(Z_{g1},Z_{g2},\cdots\right)$ and its distribution $H$ is \textbf{RV} at infinity with $\beta>1$
    \end{itemize}
\end{assumption}
