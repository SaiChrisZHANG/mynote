...
\begin{itemize}
    \item \myhl[myblue]{\textbf{Degenerate cases}}: consider the simple $k=2$ situations where $K(f,f)=0, \forall f\in\mathcal{F}$\footnote{This degeneracy appears if the variables in the array are actually \textbf{i.i.d.} in which case $\sqrt{n}\mathbb{G}_n$ converges to a Guassian process with covariance kernel $K(f_1,f_2)=\mathrm{Cov}\left(f_1(Y_{1,2}),f_2(Y_{1,2})\right)$. \citet{menzel2021bootstrap} assumes a special case where $Y_{i_1,i_2}=X_{i_1}X_{i_2}$, with $\left(X_i\right)_{i\in\mathbb{N}^+}$ i.i.d. with $\mathbb{E}\left(X_1\right)=0$, $\mathbb{V}\left(X_1\right)=1$, also $\mathcal{F}=\left\{ f_{\lambda}(x)=\lambda x,\lambda\in I \right\}$ for a compact $I\subset \mathbb{R}$.}. 
\end{itemize}
