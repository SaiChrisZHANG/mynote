...
\begin{itemize}
    ...
    \end{itemize}
    \item \myhl[myblue]{\textbf{Calculation}}:
    \begin{itemize}
...
\begin{itemize}
    \item \myhl[myblue]{\textbf{paired}}-clustered variance estimators (PCVEs):
    \begin{align*}
    ...
    \end{align*}
    \item \myhl[myblue]{\textbf{unit}}-clustered variance estimators (UCVEs):
    \begin{align*}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{clustering}: the regression has $2P\bar{n}_u$ observations and $P+1$ regressors, where $\bar{n}_u$ denotes the average number of observations across all units, then $\frac{n-1}{n-K}=\frac{2P\bar{n}_u-1}{2P\bar{n}_u-(P+1)}\simeq \frac{2\bar{n}_u}{2\bar{n}_u-1}$. This shows that the larger the average number of observations across units, the small the DOF adjustment.
    \end{itemize}
    overall, in clustered paired RCTs with more than 10 observations per unit, DOF-adjusted PCVE is preferred than DOF-adjusted UCVE; in clustered paired RCTs with strictly fewer than 10 observations per unit, PCVE \textbf{without} the DOF adjustment is preferred.
\end{itemize}

\subsubsection{Whether to Include Pair Fixed Effects}
\begin{itemize}
    \item \textbf{Non-clustered} experiments and \textbf{clustered} experimentss where in each pair the two randomization units have the same number of observations $(n_{1p}=n_{2p}),\forall p$
    \begin{itemize}
        \item \underline{without attrition}: adding pair FEs does not change the treatment coefficient or its variance, $\hat{\tau}=\hat{\tau}_{fe}$
        \item \underline{with attribution}: $\hat{\tau}$ only leverage observations from pairs where both randomization units are observed, while $\hat{\tau}$ also leverage obs. from pairs where only one of the randomization units is observed. There is \textbf{no} strong argument for or against adding pair FEs.
    \end{itemize}
    \item \textbf{Clustered} experiments where there are pairs where the two randomization units have different number of observations $\exists p$ s.t. $(n_{1p}\neq n_{2p})$, pair FEs change treatment coefficients $\hat{\tau}\neq \hat{\tau}_{fe}$
    \begin{itemize}
        \item $\hat{\tau}$ converges toward $\tau$
        \item $\hat{\tau}_{fe}$ converges toward a variance weighted average of the average treatment effect in each pair, hence it might be biased, while often with lower variance.
    \end{itemize}
\end{itemize}
