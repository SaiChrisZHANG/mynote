...
Consider two-way clustering on $G$ and $H$, for vectors $\left\{\mathbf{W}_i\right\}^n_{i=1}$, where $\mathbf{W}_i \coloneq  ( W_{i1},\cdots, W_{iK} )'\in \mathbb{R}^K $ and $i$ is the unit of observation, for the population of size $n$. The goal is to establish a central list theorem (CLT) for a 
