...
\end{align*}
and consider the restrictions on $\mathcal{F}$: for any $\eta>0$ and any seminorm $\left\Vert \cdot \right\Vert$ on a space containing $\mathcal{F}$, let $N\left(\eta, \mathcal{F}, \left\Vert \cdot \right\Vert\right)$ denotes the minimal number of $\left\Vert \cdot \right\Vert$-closed balls of radius $\eta$ with centers in $\mathcal{F}$ needed to cover $\mathcal{F}$, $N_{[]}\left(\eta, \mathcal{F}, \left\Vert \cdot \right\Vert\right)$ denotes the minimal number of $\eta-$brackets needed cover $\mathcal{F}$, where an $\eta-$bracket for $f\in\mathcal{F}$
