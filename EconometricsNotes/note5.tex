...
\begin{itemize}
    ...
    \item \myhl[myblue]{\textbf{An alternative bootstrap process}}: \citet{davezies2021empirical} established that under Assumption \ref{assump:jexch_diss_array} and \ref{assump:measurability_assumption}, if $Pf^2<\infty$ for all $f\in \mathcal{F}$ and $\mathcal{F}$ admits an envelope $F$ s.t. $PF^{1+\delta}<\infty$ for some $\delta>0$, then conditional on $\left(Y_{\mathbf{i}}\right)_{\mathbf{i}\in\mathbb{I}_k}$ and outer almost surely, the process $\mathbb{G}_n^*$ converges weakly in $l^{\infty}\left(\mathcal{F}\right)$ to a centered Gaussian process $\mathbb{G}$, the process $\mathbb{G}_n$ also converges weakly in $l^{\infty}(\mathcal{F})$ to $\mathbb{G}$.
\end{itemize}
