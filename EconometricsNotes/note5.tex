...
\begin{itemize}
    ...
    \item \myhl[myblue]{\textbf{Separately exchangeable arrays}} For the case of separately exchangeable arrays (the $n$ units stem from $k$ \textbf{different} populations, or multiway clustering as in \citet{menzel2021bootstrap}), then the assumptions must be strengthened
    \begin{assumption}{Stronger assumptions for separately exchangeable arrays}{strong_sep_exc_array}
        Consider randome variables $Y_{\mathbf{i}}$ where $\mathbf{i}=\left(i_1,\cdots,i_k\right) \in \mathbb{N}^{+k}$, implying that repetitions are allowed.
    \end{assumption}
\end{itemize}
