...
\begin{itemize}
    ...
    \item \myhl[myblue]{\textbf{Sampling}}: denote the population network as $G_{\infty,\infty}$, and let $\mathcal{V}_c,\mathcal{V}_p$ denote the set of consumers and products \textbf{randomly} sampled from $G_{\infty,\infty}$. Assume an observed matrix $\mathbf{Y}=\left[Y_{ij}\right]_{1\leq i \leq N,1\leq j\leq M}$, it is assumed to be equal
\end{itemize}
