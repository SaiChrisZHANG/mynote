...
\begin{itemize}
  ...
  \item[C] \underline{proof of statement (i)}: Proof by contradiction, if it's not true, then 
  $$
  \exists \tilde{\pi}\ \text{s.t. } $\mathrm{Pen}(\tilde{\pi})\leq \frac{C}{\beta_{\delta}^{mod}}\equiv t_{\lambda}$
  $$
  
   and that $\lVert w-Z\pi^{mod}_{\delta} \rVert_2>0$, otherwise a strictly large $\beta$ could be achieved by multiplying $\pi^{mod}_{\delta}$ by $1-\eta$ for $\eta>0$ small enough.
