...
\begin{itemize}
    \item False sign rate $\mathrm{FS}\left(\hat{\boldsymbol{\beta}}\right) = O\left( \kappa^{-4} \left(\frac{\lambda_0}{\lambda}\right)^2 s \right)$  is asymptotically vanishing when $\lambda_0/\lambda\rightarrow 0$, outperforming Lasso, whose false sign rate is generally bounded by $O\left( \lambda_{\max} \right)$ with $\lambda_{\max}$ being the largest eigenvalue of Gram matrix $n^{-1}\mathbf{X}^{\prime}\mathbf{X}$
\end{itemize}
