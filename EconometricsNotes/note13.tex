...
\begin{itemize}
    ...
    \item Convergence rates of $\frac{1}{\sqrt{n}}\left\Vert \mathbf{X}\left( \hat{\boldsymbol{\beta}} -\boldsymbol{\beta}_0 \right) \right\Vert _2$ and $\left\Vert \hat{\boldsymbol{\beta}}-\boldsymbol{\beta}_0 \right\Vert _q$ are the same as those in \citet{bickel2009simultaneous} for the $L_1$-component, and are consistent with the concave component of \citet{zhang2012general}. The bounds $O(\kappa^{-2}\lambda_0 s^{1/q}), O\left( \kappa^{-4} \left(\frac{\lambda_0}{\lambda}\right)^2 s \right)$
\end{itemize}
