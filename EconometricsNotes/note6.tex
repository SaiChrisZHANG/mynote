...
where the nuclear norm $\left\Vert \mathbf{L} \right\Vert _*$ is the sum of the singular values $\sigma_l\left(\mathbf{L}\right)$ of the matrix $\mathbf{L}$, based on the singular value decomposition $$\mathbf{L}=\mathbf{S}\boldsymbol{\Sigma}\mathbf{R}$$
