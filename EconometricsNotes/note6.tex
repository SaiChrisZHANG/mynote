...
\begin{itemize}
    \item some variables should not be adjusted for 
    \item the expected mean squared error may be reduced by ignoring those covariates that have only weak correlation with the treatment indicator and the outcomes
\end{itemize}
including a covariate in teh adjustment procedure will not lower the asymptotically precision of ATE but could reduce precision in finite samples if the covariates are not or only weakly correlated with outcomes and treatment indicators.
