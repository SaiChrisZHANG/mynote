...
\begin{itemize}
    \item $\mathbf{v}_t\in\mathbb{R}^{d_u}$ is i.i.d. in $t$, with $\mathbb{E}\left[\mathbf{v}_t\mid \boldsymbol{\xi}_{it}\right]=0$ and $\mathbb{E}\left[\mathbf{v}_t\mathbf{v}_s'\mid \boldsymbol{\xi_{it},\xi_{js}}\right] = \boldsymbol{\Sigma}_v\cdot\mathbf{1}_{t=s}$
\end{itemize}
The GLS estimateor is the best linear unbiased estimator, it has the smallest variance, and there is an explicit formula for the variance and precision of GLS in terms of $z_{it}$.
