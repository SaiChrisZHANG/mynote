...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item[3] estimate the same regression function for the controls, but using \textbf{only those that are used as matches} for the treated units with weights corresponding to the number of times a control observation is used as a match. \textbf{\underline{CAUTIOUS}}: This approach can be less efficient due to dropping some control observations, but can likely avoid including outliners. 
    \end{itemize}
\end{itemize}
