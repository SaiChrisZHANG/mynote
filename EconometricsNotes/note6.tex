...
\begin{align*}
    Q \left(R, \mathbf{U,V}, \boldsymbol{\alpha},\boldsymbol{\beta},\lambda\right) \equiv &= \sum^N_{i=1}\sum^T_{t=1} \left(1-W_{it}\right) \left(Y_{it}\sum^R_{r=1}U_{ir}V_{ir}-\alpha_i-\beta_t\right)^2 + \lambda \left(\sum^N_{i=1}\sum^R_{r=1}U^2_{ir}\right)
\end{align*}
