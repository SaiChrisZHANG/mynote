...
\begin{align*}
    ...
    & + \sum^{n-3}_{k=0} 2 \left[\bar{Y}_k(\mathbf{1}_{m+1})+ \bar{Y}_k(\mathbf{0}_{m+1})\right] \cdot \left[\bar{Y}_{k+1}(\mathbf{1}_{m+1})+ \bar{Y}_{k+1}(\mathbf{0}_{m+1})\right] 
\end{align*}
where $\bar{Y}_k(\mathbf{1}_{m+1}) = \sum^{(k+2)m}_{t=(k+1)m+1} Y_t(\mathbf{1}_{m+1})$ and $\bar{Y}_k(\mathbf{0}_{m+1}) = \sum^{(k+2)m}_{t=(k+1)m+1}Y_t(\mathbf{0}_{m+1})$. This variance has an upper bound $\mathrm{Var}^u(\hat{\tau}_m)$,
