...
\begin{itemize}
    \item \myhl[myblue]{\textbf{regression}} for population/sample/conditional ATE, we have the estimand
    \begin{equation*}
    ...
    \end{equation*}
    \begin{itemize}
        \item early estimators for $\mu_d$ included parametric regression functions including least-square estimators with the regression function $\mu_d = \beta x+\tau\cdot d$, then one can use the regression $$ Y_i =\alpha + \beta'\left(Y_{i1},\cdots,Y_{iT-1}\right) + \tau W_i +\epsilon_i $$
        ...
        \underline{\textbf{cons}}: the regression estimators may rely heavily on extrapolation, hence sensitive to changes in the specification of the models.
        \item non-parametric estimators:
    \end{itemize}
    \item \myhl[myblue]{\textbf{matching}}
    \item \myhl[myblue]{\textbf{propensity score}}
    \item \myhl[myblue]{\textbf{mixed}}
    \item \myhl[myblue]{\textbf{Bayesian}}
\end{itemize}
