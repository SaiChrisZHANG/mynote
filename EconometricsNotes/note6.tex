...
\begin{itemize}
    \item $l=0$ and only time fixed effects: $Y_{it}=\beta_t+\tau_0z_{it}+\epsilon_{it}$, then $\mathrm{Prec}\left(\hat{\tau}_0\right) = N/\sigma^2_{\epsilon} \cdot \left[T-\sum^T_{t=1}\omega^2_t\right] $
\end{itemize}
