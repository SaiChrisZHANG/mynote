...
Consider data for $J+1$ units over $T$ periods: $j=1,2,\cdots,J+1$, w.l.o.g., assume the first unit $(j=1)$ is the treated unit, and the first $T_0$ periods are before the intervention. For each unit $j$, we observe outcome $Y_{jt}$ and predictor vector $\mathbf{X}_{j}$
