...
\begin{assumption}{\citet{borusyak2020non}'s Assumptions}{borusyak2020non_assump1}
    \begin{itemize}
        \item[\textbf{A1}] \myhl[myblue]{Shock exogeneity}: $g\bot \epsilon\mid w$\footnote{The shock exogeneity combines two conditions: exclusion restriction that the realization of shocks affect outcomes via $x_l$ only; the as-good-as-random assignment of shocks w.r.t. $\epsilon$.}
        \item[\textbf{A2}] \myhl[myblue]{Weak shock exogeneity}: $\mathbb{E}\left[\epsilon_l\mid g,w\right]=\mathbb{E}\left[\epsilon_l\mid w\right]$ a.s. $\forall l$; $\mathbb{E}\left[\epsilon_l\epsilon_m\mid g,w\right] = \mathbb{E}\left[\epsilon_l\epsilon_m\mid w\right]$ a.s. $\forall l,m$
    \end{itemize}
\end{assumption}
...
when the shocks are determined by a known randomization protocol (RCT), $G(\cdot)$ is straight forward; when randomization of shocks occurs naturally, $G(\cdot)$ can come from the knowledge; in observational data, specifying $G(g\mid w)$ makes explicit the features of shocks.

\subparagraph*{Inference}
To make meaningful inferences, consider a $\mathcal{T}(g,y-bx,w)$ rationalized by $\hat{\beta}$ as 
\begin{lemma}{\citet{borusyak2020non}'s Lemma 2}{borusyak2020non_lemma2}
    Let $T= \mathcal{T}(g,y-bx,w)$ and $T^* = \mathcal{T}(g^*,y-bx,w)$, where $g^*$ is distributed according to $G(\cdot\mid w)$, independent of $(g,x,y)$, conditional on $w$. Define the Hodges-Lehmann estimator as $b\in \mathbb{R}$ that solve $T=\mathbb{E}\left[T^*\mid y,x,w\right]$. Then the recentered IV estimator is the Hodges-Lehmann estimator associated with $T=\frac{1}{L}\sum_l \left(f_l(g,w)-\mu_l\right)\left(y_l-bx_l\right)$
\end{lemma}
That is, the recentered IV estimator of $\beta$ equates the sample covariance between the recentered instrument $\tilde{z}_l$ and implied residiual $y_l-bx_l$ with the expectation of its randomization distribution.

The inference results can also be used to perform falsification tests:
\begin{itemize}
    \item for any variable $r_l$ satisfying $g\bot r\mid w$, $\tilde{z}_l$ is orthogonal to $r_l$: one can check if the sample covariance $\frac{1}{L}\sum_l\tilde{z}_l r_l$ is sufficiently close to 0
    \item falsification tests are useful that 
    \begin{itemize}
        \item when $r_l$ is a lagged outcome or a proxy for $\epsilon_l$, one can use RI for placebo and covariate balance tests of A1 \ref{assump:borusyak2020non_assump1}
        \item RI test will have power to reject false specifications of the shock assignment process (\ref{assump:borusyak2020non_assump2})
    \end{itemize}
\end{itemize}

To minimize the asymptotic variance of the recentered IV, one can use the following instrument\footnote{Such optimal instument may not be feasible in practice, but still provides guidance for constructing recentered instrument.}:
\begin{equation*}
    z^* = \mathbb{E}\left[\epsilon\epsilon'\mid w\right]^{-1} \left(\mathbb{E}[x\mid g,w]-\mathbb{E}[x\mid w]\right)
\end{equation*}
This optimal instrument satisfies
\begin{equation*}
    z^* = \left(\mathrm{Var}\left[\epsilon\mid w\right]\right)^{-1}\left[ \left(  \mathbb{E}\left[x\mid g,w\right] - \mathbb{E}\left[x\mid w\right] \right) - \nu\rho \mathbb{E}\left[\epsilon\mid w\right]  \right]
\end{equation*}
where 
\begin{align*}
    \rho &= \frac{ \mathbb{E}\left[\epsilon\mid w\right]' \Omega \tilde{z} }{\mathbb{E}\left[\epsilon\mid w\right]' \Omega^{-1} \mathbb{E}\left[\epsilon\mid w\right]} & \nu & = \frac{\mathbb{E}\left[\epsilon\mid w\right] \Omega^{-1} \mathbb{E}\left[\epsilon\mid w\right]}{ 1+ \mathbb{E}\left[\epsilon\mid w\right]'\Omega^{-1} \mathbb{E}\left[\epsilon\mid w\right]}
\end{align*}
$\rho \mathbb{E}\left[\epsilon\mid w\right]$ is the $\Omega^{-1}$-weighted projection of $\tilde{z}$ on $\mathbb{E}\left[\epsilon\mid w\right]$. This gives an intuitive 4-step description of $z^*$:
\begin{itemize}
    \item[1] take the best predictor of treatment given by the shocks and predetermined variables $\mathbb{E}\left[x\mid g,w\right]$ 
    \item[2] receter this predictor by $\mathbb{E}\left[x\mid w\right]$ to remove the potential OVB from non-random shock exposure, get $\tilde{z}$
    \item[3] partially residualizes the recentered instrument on the predictable component of the residual $\mathbb{E}\left[x\mid w\right]$
    \item[4] adjust for the residual variance $\Omega$, as in GLS.
\end{itemize}
where Step 2 and 3 are new to address the potential endogeneity of $w$.

Here, Step 1 is trivial when $x_l$ is a function of $(g,w)$: $\mathbb{E}\left[x_l\mid g,w\right] = x_l$; otherwise, $z_l$ can be given by an economic model: if $x_l = \tilde{f}_l(g,w,u)$ for a known $\tilde{f}_l(\cdot)$ and a set of unobserved shocks $u$, a reasonable stand-in for $\mathbb{E}[x_l\mid g,w]$ can be obtained by $\tilde{f}_l(g,w,0)$. Step 3 and 4 might be difficult to implement, one need to control predetermined variables in Step 3 to improve efficiency by reducing residual variance. Step 4 is more standard for correcting heteroskedasticity and mutual correlation of residuals.

\subparagraph{Practical applications}
\begin{itemize}
    \item \myhl[myblue]{\textbf{Transportation infrastructure}} instrument MA growth by recentered MA growth, the recentering is done by permutating the opening status of transportation\footnote{In the example, \citet{borusyak2020non} propose to use certain features of the transportation network, for example, the timing of line completion, as realizations of a natural experiment.}.
    \item \myhl[myblue]{\textbf{Policy eligibility}} suppose $\beta$ captures the causal effect of eligibility $x_l\in\left\{0,1\right\}$ of individual $l$ for a policy, eligibility is a function of regional government policy and individual characteristics. One way to debias is to use a regional generosity $h(g_n)$ of state $n$ as the instrument (the policies arise in a natural experiment). \citet{borusyak2020non} suggest to write the eligibility as $x_l = f(g_{state_l};v_l)$ where $v_l$ denotes the demographics relevant to the policies, and $f(\cdot)$ is a known mapping. One can debias the OLS of $y_l$ on $x_l$ by calculate each individual's expected eligibility over the possible policy counterfactuals: $$ \mu_l =\mathbb{E}\left[x_l\mid w\right] = \frac{1}{N}\sum^N_{n=1}f(g_n;v_l) $$, and either instrumenting $x_l$ by $\tilde{z}_l = x_l-\mu_l$ or controlling for $\mu_l$. $\tilde{z}_l$ also increases power comparing to the other way comes from the better first-stage prediction of $x_l$.
    \item \myhl[myblue]{\textbf{Spillover regressions}}
    \item \myhl[myblue]{\textbf{Shift-share instruments}}
    \item \myhl[myblue]{\textbf{Model-implied optimal instrumetnts}}
    \item \myhl[myblue]{\textbf{Other spatial instruments}}
\end{itemize}
