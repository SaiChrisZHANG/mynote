...
\begin{itemize}
    \item \myhl[myblue]{\textbf{unit FE estimator}} $\hat{\beta}_{UFE} = \sum^N_{i=1}\sum^T_{t=1}\tilde{Y}_{i,t} \mathring{Y}_{i,t}\mathring{W}_{i,t}/\sum^N_{i=1}\sum^T_{t=1}\mathring{W}^2_{i,t} $: Define $\mathring{\mu}_{i,t}\coloneq \mathbb{E}[\mathring{W}_{i,t}\mid\mathcal{F}_{1:N,0,T}]$, $\mathrm{Var}(\ddot{W}_{i,t}\mid \mathcal{F}_{1:N,0,T})\coloneq \ddot{\sigma}^2_{W,i,t}<\infty$ $\mathrm{Cov}(\mathring{W}_{i,t},\mathring{W}_{i,s}\mid\mathcal{F}_{1:N,0,T}) \coloneq \mathring{\sigma}_{W,i,t,s}$, and assume that 
    \begin{align*}
    ...
    \begin{itemize}
        \item ${\sum^T_{t=1}\ddot{\kappa}_{W,\beta,t,t}}/{\sum^T_{t=1}\ddot{\sigma}^2_{W,t}}$: how contemporaneous dynamic causal coefficients covary with the within-unit transformed assignments
        \item ${\sum^T_{t=1}\sum^{t-1}_{s=1}\ddot{\kappa}_{W,\beta,t,s}}/{\sum^T_{t=1}\ddot{\sigma}^2_{W,t}}$: how past causal coefficients covary with the within-unit transformed treatments due to dynamic causal effects
        \item ${\sum^T_{t=1}\ddot{\delta}_t}/{\sum^T_{t=1}\ddot{\sigma}^2_{W,t}}$: error due to the possible relationship between the demaned counterfactual $\ddot{Y}_{i,t}(\mathbf{0})$ and the demeaned treatment assignment
    \end{itemize}
\end{itemize}
