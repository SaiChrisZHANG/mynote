...
\begin{itemize}
    ...
    and estimate a low-rank approximation $\left\{\hat{M}_{jt}\right\}$ to the true matrix $\left\{M_{jt}\right\} = \left\{f(\mu_j,\lambda_t)\right\}$, and the $\hat{M}_{jt}$ are used to de-noise the outcomes $Y^N_{jt}$ and to impute missing values, and obtain synthetic controls as linear combinations of $\hat{M}_{jt}$
\end{itemize}
