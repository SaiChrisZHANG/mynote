...
\begin{definition}{lag-$p$ dynamic causal effect}{bojinov2021panel_lagp}
    For $0\leq p <t$ and $\mathbf{w},\tilde{\mathbf{w}}\in\mathcal{W}^{p+1}$, the $i,t$th lag-$p$ dynamic causal effect is 
    \begin{equation*}
    ...
    For integers $p,q$ satisfying $0\leq p <t,0<q\leq p+1$, the weighted average $i,t$th lag-$p,q$ dynamic causal effect is
    ...
    where $\mathbf{w},\tilde{\mathbf{w}}\in \mathcal{W}^q$ and $\left\{a_{\mathbf{v}}\right\}$ are nonstochastic weights satisfying $\sum_{\mathbf{v}\in \mathcal{W}^{p-q+1}} a_{\mathbf{v}}=1$, and $a_{\mathbf{v}}\geq 0$ for all $\mathbf{v}\in\mathcal{W}^{p-q+1}$.
\end{definition}
