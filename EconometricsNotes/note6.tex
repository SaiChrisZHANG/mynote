...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item focus on an average treatment effect that is estimable with greater accuracy
        \begin{itemize}
            \item \textit{\underline{drop observations by a \textbf{cutoff} of propensity scores}}: one principle is to evaluate the weight of each unit as
            \begin{align*}
                \frac{1}{N\cdot \left[1-e(X_i)\right]}, &\text{ for treated units} & \frac{1}{N\cdot e(X_i)}, &\text{ for control units}
            \end{align*}
            and make sure the weights of all units are \textbf{upper-bounded} by a reasonable number\footnote{For example, set the cutoff as 0.05, s.t. no unit will have a weight of more than 5\% in the average.}
        \end{itemize}
    \end{itemize}
\end{itemize}
