...
For inference, \citet{callaway2021difference} focuses on the doubly-robust (DR) approach, and get the estimators
\begin{align*}
    \hat{ATT}^{nev}_{DR}\left(a,t;\delta\right) &= \mathbb{E}_n \left[ \left(\hat{w}^{treat}_a-\hat{w}^{comp,nev}_a\right) \left(Y_{t}-Y_{a-\delta-1} -\hat{m}^{nev}_{a,t,\delta} \left(X;\hat{\beta}^{nev}_{a,t,\delta}\right) \right) \right] \\
    \hat{ATT}^{ny}_{DR}\left(a,t;\delta\right) &= \mathbb{E}_n \left[ \left(\hat{w}^{treat}_a-\hat{w}^{comp,nev}_a\right) \left(Y_{t}-Y_{a-\delta-1} -\hat{m}^{nev}_{a,t,\delta} \left(X;\hat{\beta}^{nev}_{a,t,\delta}\right) \right) \right]
\end{align*}
