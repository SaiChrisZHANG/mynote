...
One way to choose $\mathbf{W}$ is, for nonnegative $v_1,\cdots,v_k$, to \textbf{minimize}
\begin{equation*}
    \mathbf{W}^* = \arg\max_{\mathbf{W}} \left\Vert \mathbf{X}_1 - \mathbf{X}_0\mathbf{W} \right\Vert = \arg\max_{\omega_{j}} \sqrt{ \sum^k_{h=1}v_h \left(X_{h1}-\omega_2 X_{h2} - \cdots - \omega_{J+1} X_{hJ+1} \right)^2 }
\end{equation*}
