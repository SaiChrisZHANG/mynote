...
Consider the T-optimal design: $$ \max_A\mathrm{tr}\left(\mathrm{Prec}\left(\hat{\boldsymbol{\tau}};A\right)\right) $$
where $\mathrm{Prec}\left(\hat{\boldsymbol{\tau}};A\right)$ is the precision matrix of $\boldsymbol{\tau}$ (the matrix inverse of the var-cov matrix of $\boldsymbol{\tau}$) when $A$ is used.
