...
\begin{itemize}
    \item $\Delta Y_t$ for the untreated and $\Delta Y_{t-1},Y_{t-1},Y_{t-2}$ for the treated are \textbf{continuously} distributed with densities that are uniformly bounded from above and away from 0.
    \item The observed data $\left\{ Y_{it},Y_{it-1},Y_{it-2},X_i,D_i \right\}^n_{i=1}$ are i.i.d. draws from the joint distribution $F_{Y_t,Y_{t-1},Y_{t-2},X,D}$ and $Y_{it}=D_iY_{1it}+(1-D_i)Y_{0it}$, $Y_{it-1}=Y_{0it-1}$ and $Y_{it-2}=Y_{0it-2}$
\end{itemize}
