...
\begin{itemize}
    \item No covariates, $\eta_i$'s effects are the same in both periods, $$ \hat{f}_{Y^0_{i2}\mid D_i=1}(y) = \frac{1}{2\pi} \int^{T_N}_{-T_N} \exp\left(-ity\right) \times \left[ \frac{\hat{\Psi}_{Y_{i1}\mid D_i=1}(t)}{\hat{\Psi}_{Y_{i1}\mid D_i=0}(t)}\hat{\Psi}_{Y_{i2}\mid D_i=0}(t) \right] \mathrm{d}t $$
\end{itemize}
where $\hat{\Psi}_W$ denotes the empirical characteristic function of $W$\footnote{For example, $\hat{\Psi}_{Y_{i2}\mid D_i=0}(t) = \frac{1}{N_0}\sum_{i,D_i=0}\exp(itY_{i2})$.}.
